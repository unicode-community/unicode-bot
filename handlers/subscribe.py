...
async def subscribe(callback: types.CallbackQuery, state: FSMContext, db: Database, cfg: Config) -> None:
    ...
    if subscr_info["is_subscriber"]:
        ...
        if not subscr_info["is_subscribed_to_payments"]:
            ...
            txt += "\n\n" + messages.last_active_time_subscr.format(
                date=subscr_info["subscription_end"].astimezone(timezone("Europe/Moscow")).strftime("%d.%m.%Y %H:%M")
            )
            txt += "\n\n" + messages.add_for_temp_subscr
            idempotence_key = str(uuid.uuid4())
            payment = Payment.create(
                create_subscription_params(price=cfg.subscription_price, return_url=cfg.bot_link, user_id=callback.from_user.id), idempotency_key=idempotence_key
            )
            await callback.message.answer(
                text=txt,
                reply_markup=keyboards.create_kb_to_payment(
                    url=payment.confirmation.confirmation_url, payment_id=payment.id
                ),
            )
        else:
            # СЦЕНАРИЙ "ЕСЛИ АКТИВНА ПОСТОЯННАЯ ПОДПИСКА"
            txt += "\n\n" + messages.next_pay_date.format(
                date=subscr_info["subscription_end"].astimezone(timezone("Europe/Moscow")).strftime("%d.%m.%Y %H:%M")
            )
            await callback.message.answer(text=txt, reply_markup=keyboards.break_subscr_and_return_to_menu)
...
async def confirm_break_subscr(callback: types.CallbackQuery, state: FSMContext, db: Database) -> None:
    ...
    await callback.message.answer(text=messages.succesful_break_subscr, reply_markup=return_to_menu)
    await callback.answer()


@router.callback_query(F.data.startswith("check_payment_"))
async def check_payment(callback: types.CallbackQuery, db: Database, bot: Bot, cfg: Config) -> None:
    payment_id = callback.data.split("_")[2]
    payment = Payment.find_one(payment_id)
    if payment.status == "succeeded":
        try:
            user_info = await get_subscription_status(user_tg_id=callback.from_user.id, db=db)
            if (user_info["subscription_end"] is not None) and (
                datetime.now(tz=timezone("Europe/Moscow")) <= user_info["subscription_end"]
            ):
            ...
            await callback.message.answer(text=messages.successful_pay_subscr, reply_markup=return_to_menu)
            ...
            await callback.message.answer(
                ...
                reply_markup=keyboards.create_kb_to_payment(
                    url=payment.confirmation.confirmation_url, payment_id=payment.id, add_write_support=True
                ),
            )
        ...
        await callback.answer(text="⏳ Платеж создан и ожидает действий от пользователя.")
    await callback.answer()
