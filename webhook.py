...
async def check_pay_yookassa(request: Request, db: Database = Depends(get_db)):
    ...
    if (user_info["subscription_end"] is not None) and (datetime.now(tz=timezone("Europe/Moscow")) <= user_info["subscription_end"]):
    ...
    if "auto_pay" in payment_object["metadata"]:
        ...
        admin_text = (
            f"Подписчик @{user.tg_username} получена автоплата\n"
        ...
        admin_text = (
            f"У вас появился новый подписчик @{user.tg_username}\n"
            f"Цена: {data['object']['amount']['value']} руб\n"
            f"Период оплаты: 30 дней"
        )
